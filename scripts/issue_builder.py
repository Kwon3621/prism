...
3. 서로 다른 언론사 기사가 2건 이상 걸리는 키워드만 포함하세요.
   ...
   기사를 끼워 넣지 마세요.
4. 키워드에 속한 기사들이 같은 배경·원인은 공유하지만 각자 다른
   구체적 결과·영향을 다루는 경우(예: "반도체 호황"이 원인이지만
   한 기사는 그로 인한 금리 인상과 영끌족 부담을, 다른 기사는 그로
   인한 제조업 반도체 쏠림 문제를 다루는 경우), 그 원인 하나만으로
   키워드를 짓지 말고 "~로 인한 경제 현상", "~ 관련 파급효과"처럼
   여러 결과를 함께 아우르는 표현으로 지으세요(예: "반도체 경기 호황"
   대신 "반도체 경기 호황으로 인한 경제 현상"). 기사들이 하나의
   구체적 결정·사건을 그대로 공유한다면(예: "정청래 후원회장 인선")
   기존처럼 그 사건명을 그대로 쓰세요 — 굳이 이 표현을 덧붙이지
   마세요.
5. 키워드마다 그 키워드에 속한 기사들이 공통으로 다루는 사실만으로
   1~2문장의 summary를 작성하세요. 이 summary는 재검색 없이 그대로
   사용자에게 노출됩니다.
6. 키워드는 최대 {MAXIMUM_KEYWORD_COUNT}개까지, 언급 비중(기사 수·언론사
   수)이 큰 순서로 정렬하세요.
7. 내부 판단 과정은 출력하지 말고 최종 JSON만 반환하세요.
8. Markdown 코드 블록이나 JSON 이외의 문장은 출력하지 마세요.
