...
1. 각 기사가 공통된 하나의 중심 사건, 결정, 발표, 사고 또는 변화를 명확한 출발점으로 삼고 있어야 합니다.
2. 기사의 주요 인물, 기관, 대상 또는 세부 논점이 서로 다르더라도, 동일한 중심 사건의 사실, 원인, 배경, 영향, 반응, 의미, 전망 또는 후속 조치를 직접 다루는 경우에는 같은 event group에 포함하세요.
3. 각 기사의 제목이나 설명에서 동일한 중심 사건을 직접 언급하거나, 해당 사건의 원인, 배경, 영향, 반응, 의미, 전망 또는 후속 조치와의 연결을 확인할 수 있어야 합니다.
4. 발생 시점과 장소는 중심 사건의 동일성을 부정할 정도로 모순되지 않아야 합니다. 기사 작성일이나 후속 보도 시점이 다르다는 이유만으로 별개의 사건으로 판단하지 마세요.
5. 공통된 검색어, 인물, 기관, 기업, 산업 또는 주제만 있을 뿐, 서로를 연결하는 구체적인 중심 사건이 없다면 다른 그룹으로 분리하거나 제외하세요.

[반드시 분리하거나 제외해야 하는 경우]

1. 기사들이 같은 넓은 주제, 인물, 기관, 기업, 산업 또는 지역을 다루더라도, 각 기사가 출발점으로 삼는 구체적인 중심 사건이 다르면 서로 다른 event group으로 분리하세요.
2. 검색어 "{query}"가 단순히 언급되었을 뿐 기사의 핵심 내용과 무관하면 어떤 group에도 포함하지 마세요.
3. 배경 설명, 전망 또는 해설 기사도 그룹의 중심 사건을 바탕으로 그 원인, 배경, 영향, 반응, 의미, 전망 또는 후속 상황을 직접 다루면 같은 event group에 포함할 수 있습니다. 다만 기사 제목과 설명에서 특정 중심 사건과의 직접적인 연결을 확인할 수 없고, 일반적인 주제나 장기적 흐름만 다루는 경우에는 제외하세요.
4. 제목의 일부 단어, 검색어 또는 고유명사가 같다는 이유만으로 같은 사건으로 묶지 마세요.
5. 각 기사가 동일한 중심 사건을 직접 언급하거나, 해당 사건의 원인·배경·영향·반응·의미·전망·후속 조치 중 하나를 명확히 다루는 경우에만 같은 event group에 포함하세요.
